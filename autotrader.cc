...
constexpr ulong DELTA = 100;
constexpr ulong PILLOW = 100;
constexpr long VOLUME = 20;
constexpr long CAP = 3;  // For a CAP >3 , you have to be worried about message
                         // limits (from all the cancels sent)
constexpr ulong K = 5;
...
void AutoTrader::HedgeFilledMessageHandler(unsigned long clientOrderId,
    ...
    if (it != mHedges.end()) {
        ...
        if (price == 0 && volume == 0) {
            ...
            if (side == Side::BUY) {
                mHedges[mNextMessageId] = std::make_tuple(
                    Side::SELL, MAX_ASK_NEAREST_TICK, VOLUME, unhedge);

                SendHedgeOrder(mNextMessageId++, Side::SELL,
                ...
                mHedges[mNextMessageId] = std::make_tuple(
                    Side::BUY, MIN_BID_NEARST_TICK, VOLUME, unhedge);
        ...
        if (unhedge) {
            ...
            long unhedge_price = (unhedgeSide == Side::BUY)
                                     ? curETFBidPriceBook[0]
                                     : curETFAskPriceBook[0];

            mOrders[mNextMessageId] =
                std::make_tuple(unhedgeSide, unhedge_price, VOLUME, false);
            SendInsertOrder(mNextMessageId++, unhedgeSide, unhedge_price,
        ...
        RLOG(LG_AT, LogLevel::LL_INFO)
            ...
            << ": Hedged for Volume " << volume << ": Hedged for Price "
            << price << ": mETFPosition: " << mETFPosition << ": Reverse "
            << unhedge << ": mFUTPosition: " << mFUTPosition;
    } else
        RLOG(LG_AT, LogLevel::LL_INFO)
    ...
    if (instrument == Instrument::FUTURE) {
        ...
        if (mETFPosition <= POSITION_LIMIT) {
            ...
            if (bidVolETF > 0) {
                ...
                RLOG(LG_AT, LogLevel::LL_INFO)
                    << "Order side BUY placed no. " << mNextMessageId - 1
                    << " For " << Pt << " Vol " << bidVolETF;

            }

            if (mETFPosition <= (long)POSITION_LIMIT/2) {
                mOrders[mNextMessageId] =
                    std::make_tuple(Side::BUY, Pt, 25, true);
        ...
        if (mETFPosition >= -POSITION_LIMIT) {
            ...
            if (askVolETF > 0) {
                ...
                SendInsertOrder(mNextMessageId++, Side::SELL, Pt, askVolETF,
                                Lifespan::GOOD_FOR_DAY);
                RLOG(LG_AT, LogLevel::LL_INFO)
                    << "Order SELL side placed no. " << mNextMessageId - 1
                    << " For " << Pt << " Vol " << askVolETF;
            }
        }
...
void AutoTrader::OrderFilledMessageHandler(unsigned long clientOrderId,
    ...
    if (order_it != mOrders.end()) {
        ...
        // Bid or Ask
        Side hedgeSide = (std::get<0>(order_it->second) == Side::BUY)
                             ? Side::SELL
                             : Side::BUY;

        // The side is hedge side so you actually ask for opposite
        long nearest_tick = (hedgeSide == Side::BUY) ? MAX_ASK_NEAREST_TICK
                                                     : MIN_BID_NEARST_TICK;
        ...
        mHedges[mNextMessageId] =
            std::make_tuple(hedgeSide, price, volume, false);
        SendHedgeOrder(mNextMessageId++, hedgeSide, nearest_tick, volume);

        RLOG(LG_AT, LogLevel::LL_INFO)
            << "!!order " << clientOrderId << " has volume " << volume
            << " priced at" << price << " on side "
            << std::get<0>(order_it->second) << ": Position: " << mETFPosition
            << ": ASKVOL " << askVolETF << ": BIDVOL " << bidVolETF;
...
void AutoTrader::TradeTicksMessageHandler(
    ...
    RLOG(LG_AT, LogLevel::LL_INFO)
        ...
        << "; bid volumes: " << bidVolumes[0] << "; bidETF: " << bidETF
        << " askETF: " << askETF << "; Recently filled "
        << mOrderRecentlyFilled;

    // Re-price based on outstanding trades
    // bool asks = false, bids = false;
    // for (auto &[orderId, tup] : mOrders) {
    //     // Exit early if you know bids and asks are already outstanding
    //     if (asks && bids) break;

    //     if (std::get<0>(tup) == Side::SELL) asks = true;
    //     if (std::get<0>(tup) == Side::BUY) bids = true;
    // }

    // // All asks we put out are filled
    // if (!bids) bidETF = curFUTBidPriceBook[0]; //- DELTA * K;
    // if (!asks) askETF = curFUTAskPriceBook[0]; //+ DELTA * K;
}
